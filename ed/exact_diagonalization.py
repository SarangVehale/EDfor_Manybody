...
class BasisGenerator:
    ...
    @staticmethod
    def translation_representatives(basis: np.ndarray, N: int) -> Dict[int, Tuple[int, int, int]]:
        """
    ...
    @staticmethod
    def momentum_sector_basis(N: int, k: int, Sz_target: int = 0) -> Tuple[np.ndarray, Dict, np.ndarray]:
        """
...
class LanczosEngine:
    ...
    def run(self, k: int = 10, tol: float = 1e-12, maxiter: int = 500,
            reorth: bool = True) -> Tuple[np.ndarray, np.ndarray, Dict]:
        """
        ...
        for j in range(1, maxiter):
            ...
            if beta[j-1] < 1e-14:
                ...
                vecs = Q[:, :j] @ evecs
                convergence = {'converged': True, 'iterations': j, 'history': convergence_history}
            ...
            if j % 10 == 0 and j >= k + 5:
                ...
                if len(convergence_history) > 0:
                    ...
                    self.logger(f"Iteration {j}, max eigenvalue change: {max_change:.2e}")
        ...
        evals, evecs = np.linalg.eigh(T)
        vecs = Q @ evecs
        convergence = {'converged': False, 'iterations': maxiter, 'history': convergence_history}
        return evals[:k], vecs[:, :k], convergence
...
class ObservableCalculator:
    ...
    @staticmethod
    def expectation_value(basis: np.ndarray, psi: np.ndarray, 
                          operator_func: Callable) -> float:
    ...
    @staticmethod
    def sz_correlation(basis: np.ndarray, idx_map: Dict, psi: np.ndarray,
                       i: int, j: int) -> float:
        """<S^z_i S^z_j>"""
        result = 0.0
        for idx, state in enumerate(basis):
    ...
    @staticmethod
    def correlation_matrix(basis: np.ndarray, idx_map: Dict, psi: np.ndarray,
                          N: int) -> np.ndarray:
    ...
    @staticmethod
    def entanglement_entropy(basis: np.ndarray, psi: np.ndarray, 
                            N: int, subsystem_A: List[int]) -> float:
        """
    ...
    @staticmethod
    def structure_factor(basis: np.ndarray, idx_map: Dict, psi: np.ndarray,
...
class EDEngine:
    ...
    def build_basis(self):
        ...
        if self.config.model in ['heisenberg', 'tfim']:
            if self.config.use_translation and self.config.momentum is not None:
                self.basis, self.idx_map, self.norms = BasisGenerator.momentum_sector_basis(
                    self.config.N, self.config.momentum, 
                    self.config.Sz_sector or 0
                )
            else:
    ...
    def build_hamiltonian(self):
        ...
        self.logger(f"Hamiltonian built: {nnz:,} nonzeros, "
    ...
    def diagonalize(self, method: str = 'eigsh', k: int = 6, 
                   tol: float = 1e-12) -> DiagResults:
        """
        ...
        if method == 'full':
            ...
            if dim < 100:
                ...
                eigvals = eigvals[:k]
                eigvecs = eigvecs[:, :k]
                convergence = {'converged': True, 'method': 'eigsh->dense', 'iterations': 1}
            else:
    ...
    def _validate_eigenpairs(self, eigvals: np.ndarray, eigvecs: np.ndarray):
        ...
        for i in range(min(3, len(eigvals))):
            ...
            if residual > 1e-8:
                self.logger(f"WARNING: Large residual for eigenvalue {i}: {residual:.2e}")
            else:
        ...
        if eye_diff > 1e-8:
            self.logger(f"WARNING: Eigenvectors not orthonormal: ||V†V - I|| = {eye_diff:.2e}")
        else:
...
Examples:
  ...
  python exact_diagonalization_production.py --validate
    ...
    # Observables
    parser.add_argument('--compute_observables', action='store_true',
    ...
    if not args.no_save:
        ...
        if observables:
            ...
            np.savez(obs_file, **{k: v for k, v in observables.items() 
